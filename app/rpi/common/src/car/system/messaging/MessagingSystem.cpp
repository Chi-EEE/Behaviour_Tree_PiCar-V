...
namespace car::system::messaging
{
	...
	void MessagingSystem::handleMessage(const std::string& message) const
	{
		...
		try
		{
			...
			{
			case utils::hash("selection"):
			{
				if (!message_json.HasMember("selected") || !message_json["selected"].IsBool())
				{
					spdlog::error("MessagingSystem::handleMessage: No `selected` in json", message);
					return;
				}
				this->selection_signal_(message, message_json);
				return;
			}
