...
namespace behaviour_tree::node::task::action
{
	...
	public:
		...
		const std::string toString() const override {
			...
			if (name != "")
				return fmt::format(R"(<Action:SetAngle name="{}" servo="{}" angle="{}"/>)", name, servo_type, this->getAngle());
			else
				return fmt::format(R"(<Action:SetAngle servo="{}" angle="{}"/>)", servo_type, this->getAngle());
		}
